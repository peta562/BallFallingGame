...
namespace Core.Progress {
    public sealed class ProgressController : BaseController {
        ...
        public ProgressController(ProgressConfig progressConfig, ScoreController scoreController, WindowManager windowManager) {
        ...
        public void TryStartLevel() {
            if ( _currentLevelId <= _progressConfig.MaxLevels ) {
                ...
                if ( currentLevelInfo != null ) {
                    _windowManager.ShowWindow<PlayWindow>(x => x.Init(_scoreController, currentLevelInfo));
                } else {
